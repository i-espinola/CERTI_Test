...
var parts_util_addConjunction = function addConjunction(parts, _int) {
  var lastNum = getLastNumber(_int); // A parte é valida apenas se:
...
var parts_util_name = function name(parts, locale) {
  return reverse_module(reverse_module(parts).map(function (part, i) {
    var numberName = listGt1000(locale)[i - 1];
...
/* harmony default export */ var gt1000 = (function (_int, locale) {
  var number = parts_util_write(addComma(parts_util_addConjunction(singularize(clear(parts_util_name(int_util_split(_int), locale))), _int)), locale);
  return number.join(' ');
});
...
/* harmony default export */ var write_int = (function (_int, locale) {
  ...
  if (intNum > 1000) num = gt1000(_int, locale);
...
/* harmony default export */ var write_currency_write = (function (val, locale, opts) {
  var number = parseInt(val);
  var text = write_int(val, locale);
...
/* harmony default export */ var write_currency = (function (iso, locale) {
  ...
  var unitText = write_currency_write(unit, locale, opts);
...
/* harmony default export */ var write_all = __webpack_exports__["default"] = (function (num, opts) {
  ...
  if (!isValidOpt(opts.mode, ['number', 'currency']) || !isValidOpt(opts.locale, ['pt', 'br']) || !isValidOpt(opts.negative, ['formal', 'informal']) || !isValidOpt(opts.currency.type, ['BRL', 'EUR', 'ECV']) || !isValidOpt(opts.number.gender, ['m', 'f']) || !isValidOpt(opts.number.decimal, ['formal', 'informal'])) {
    throw new Error('Invalid option');
  }

  var _parseNumber = parseNumber(numString),
  ...
  if (opts.mode === 'currency') {
    ...
    var numText = write_currency(iso, locale, integer, decimal);
  ...
  if (opts.mode === 'number') {
    ...
    var intText = write_int(integer, opts.locale, opts.number.gender);
    var decText = write_decimal(decimal, opts.locale, opts.number.decimal); // Se tem a parte inteira e não tem a parte decimal

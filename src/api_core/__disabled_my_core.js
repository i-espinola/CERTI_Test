...
function toExpression(value) {
  ...
  while (numbers.length > 0) {
    const leftover = parseInt(numbers.join().replace(/[^0-9]/g, ''));
    let number = parseInt(numbers.shift());
    let partialSize = numbers.length;

    if (leftover === 100 || (leftover >= 11 && leftover <= 19)) {
      leftover === 100
        ? write.push(words.nameHundred)
        : write.push(words.names[partialSize][leftover]);
      numbers = [];
      continue;
      ...
      (fullSize === fiveDigit && fullSize === partialSize)
      || (fullSize === fiveDigit && fullSize - 1 === partialSize)
    ) {
      if (number === 1) {
      ...
      fullSize === partialSize
        ? write.push(tensOfThousands[number])
        : write.push(`${thousandUnit[number]} ${words.nameThousand}`);
      continue;
    } else if (number === 0) {
      continue;
  ...
  output = write
    .join()
    .replace(/mil,/g, `${words.nameThousand} `)
    .replace(/,/g, words.and);
  return output;
}
...
function startup(input) {
  ...
  if (validate) {
    number < 0
      ? (number = number.replace(/[^0-9]/g, '')) + (isNegative = true)
      : '';
